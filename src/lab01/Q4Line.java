...
public class Q4Line extends JPanel {
    private static final String TITLE = "Line Chart: Total Sales of Product A";
    ...
    private static final int ARROW_SIZE = 13, SEP_BAR_LENGTH = 5, POINT_SIZE = 4;
    ...
    private static void plotData(Graphics2D g) {
        ...
        for (int i = 0; i < Y_DATA.length; i++) {
            int height = (int) (Y_SEP * Y_DATA_SEP_NUM * Y_DATA[i] / Y_DATA_MAX);
            ...
            g.draw(new Line2D.Double(x - POINT_SIZE, y - POINT_SIZE, x + POINT_SIZE, y + POINT_SIZE));
            g.draw(new Line2D.Double(x + POINT_SIZE, y - POINT_SIZE, x - POINT_SIZE, y + POINT_SIZE));

            // Text
            float xText = x - (float) (g.getFontMetrics().stringWidth("" + Y_DATA[i]) / 2.0);
            g.drawString("" + Y_DATA[i], xText, y - 8);
